...
export function DashboardPageHeader({
    ...
    return (
        <div className="border-b border-base-300/60 px-6 py-5 lg:px-8">
            {backButton ? <div className="flex flex-wrap items-center gap-3">{backButton}</div> : null}

            <div className={`${backButton ? 'mt-5 ' : ''}space-y-5`}>
                <div className="flex flex-col gap-4">
                    <div className="space-y-3">
                        {meta ? <div className="flex flex-wrap items-center gap-2 text-sm text-base-content/60">{meta}</div> : null}
